...
export default function BookingFlow({ services }: { services: ServiceData[] }) {
  ...
  const handleServiceSelect = (serviceSlug: string) => {
    setBookingData(prev => ({ ...prev, service: serviceSlug }))
  }
            ...
            {selectedService && (
              ...
              >
                {selectedService.price ? (
                  ...
                  <p className="text-sm text-[--color-text-muted]">Prezzo da confermare</p>
                )}
                <p className="text-xs text-[--color-text-muted]">{formatDuration(selectedService.duration)}</p>
              </motion.div>
            )}
          </div>
                      ...
                      {services.map((service) => (
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            ...
                            {service.price ? (
                              <span className="text-[--color-accent] font-semibold">&euro;{service.price}</span>
                            ) : null}
                          </div>
                          <p className="text-xs text-[--color-text-muted]">{formatDuration(service.duration)}</p>
                          {service.shortDescription && (
                            <p className="text-xs text-[--color-text-muted] mt-1 line-clamp-2">{service.shortDescription}</p>
                          )}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="space-y-3 mb-6">
                    ...
                    {selectedService?.price && (
                      <div className="flex justify-between items-center py-3 bg-[--color-accent]/10 rounded-lg px-4">
                        ...
                        <span className="text-xl font-semibold text-[--color-accent]">&euro;{selectedService.price}</span>
                      </div>
                    )}
                  </div>
